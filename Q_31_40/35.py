...
for l in range(L):
    for k in range(K):
        G[l, k] = np.sum( gray_img * np.exp(-2j * np.pi * (k * x / W + l * y / H))) / np.sqrt(H * W)
...
# 中心からの四隅までの距離rに対してp1*r ~ p2*rのものだけを通す(１になる)マスク
...
for h in range(H):
    for w in range(W):
        out[h, w] = np.abs(np.sum( G * np.exp( 2j * np.pi * (w * x / W + h * y / H)))) / np.sqrt(H * W)
