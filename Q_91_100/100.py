...
recall_num = 0
for ans in GT:
    img = cv2.rectangle(img, (ans[0], ans[1]), (ans[2], ans[3]), (0,255,0))
    for detection in R:
        img = cv2.putText(img, "{:.2f}".format(detection[-1]), (detection[0], detection[1]+10), cv2.FONT_HERSHEY_PLAIN, 0.8, (255,0,255))
        if IoU(ans, detection[:4]) > th:
            img = cv2.rectangle(img, (detection[0], detection[1]), (detection[2], detection[3]), (0,0,255))
            recall_num += 1
        else:
            img = cv2.rectangle(img, (detection[0], detection[1]), (detection[2], detection[3]), (255,0,0))

recall = recall_num / len(GT)
